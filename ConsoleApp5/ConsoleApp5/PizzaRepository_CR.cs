...
/// <summary>
/// Представляет репозиторий для работы с данными пиццерии.
/// </summary>
public class PizzaRepository
{
    /// <summary>
    /// Получает или задаёт список клиентов.
    /// </summary>
    public List<Client> Clients { get; set; }

    /// <summary>
    /// Получает или задаёт список заказов.
    /// </summary>
    public List<Order> Orders { get; set; }

    /// <summary>
    /// Получает или задаёт список позиций заказов.
    /// </summary>
    public List<OrderItem> OrderItems { get; set; }

    /// <summary>
    /// Получает или задаёт список блюд меню.
    /// </summary>
    public List<Menu> Menu { get; set; }

    /// <summary>
    /// Инициализирует новый экземпляр класса PizzaRepository.
    /// </summary>
    ...
    public void LoadData()
    {
        try
        {
            ...
            LoadClients(workbook); // Принцип единой отвественности
            LoadOrders(workbook);
            LoadOrderItems(workbook);
            LoadMenu(workbook);
    ...
    /// <summary>
    /// Загружает клиентов из Excel-файла.
    /// </summary>
    /// <param name="workbook">Книга Excel.</param>
    private void LoadClients(Workbook workbook)
    {
    ...
    /// <summary>
    /// Загружает заказы из Excel-файла.
    /// </summary>
    /// <param name="workbook">Книга Excel.</param>
    private void LoadOrders(Workbook workbook)
    {
    ...
    /// <summary>
    /// Загружает позиции заказов из Excel-файла.
    /// </summary>
    /// <param name="workbook">Книга Excel.</param>
    private void LoadOrderItems(Workbook workbook)
    {
    ...
    /// <summary>
    /// Загружает меню из Excel-файла.
    /// </summary>
    /// <param name="workbook">Книга Excel.</param>
    private void LoadMenu(Workbook workbook)
    {
    ...
    /// <summary>
    /// Получает список клиентов из Перми.
    /// </summary>
    /// <returns>Список клиентов из Перми.</returns>
    ...
    /// <summary>
    /// Получает список невыполненных заказов с информацией о клиентах.
    /// </summary>
    /// <returns>Список строк с информацией о заказах.</returns>
    public List<string> GetOrdersWithClientInfo()
    {
        return (from order in Orders
                ...
                select $"Заказ {order.OrderID} от {order.Date:dd.MM.yyyy} - {client.LastName} {client.Name}, {client.City}").ToList();
    }

    /// <summary>
    /// Получает общий доход от выполненных заказов.
    /// </summary>
    /// <returns>Общий доход.</returns>
    public decimal GetTotalDeliveredOrdersIncome()
    {
        return (from order in Orders
                ...
                select orderItem.Quantity * dish.Price + order.Price).Sum();
    }

    /// <summary>
    /// Получает среднее количество позиций в заказе.
    /// </summary>
    /// <returns>Среднее количество позиций.</returns>
    public double GetAverageItemsPerOrder()
    {
        return (from order in Orders
                ...
                select g.Sum(oi => oi.Quantity)).DefaultIfEmpty().Average();
    }

    /// <summary>
    /// Удаляет клиента и все связанные с ним заказы.
    /// </summary>
    /// <param name="clientId">Идентификатор клиента.</param>
    /// <returns>true, если удаление выполнено успешно; иначе false.</returns>
    public bool RemoveClient(int clientId)
    {
        Client client = (from c in Clients where c.ClientID == clientId select c).FirstOrDefault();
        ...
        List<Order> ordersToRemove = (from o in Orders where o.ClientID == clientId select o).ToList();

        foreach (Order order in ordersToRemove)
        {
            List<OrderItem> orderItemsToRemove = (from oi in OrderItems
                                                  where oi.OrderID == order.OrderID
                                                  select oi).ToList();

            foreach (OrderItem item in orderItemsToRemove)
            {
    ...
    /// <summary>
    /// Удаляет заказ и все связанные с ним позиции.
    /// </summary>
    /// <param name="orderId">Идентификатор заказа.</param>
    /// <returns>true, если удаление выполнено успешно; иначе false.</returns>
    public bool RemoveOrder(int orderId)
    {
        Order order = (from o in Orders where o.OrderID == orderId select o).FirstOrDefault();
        ...
        List<OrderItem> orderItemsToRemove = (from oi in OrderItems where oi.OrderID == orderId select oi).ToList();
    ...
    /// <exception cref="ArgumentException">Выбрасывается, если клиент не существует.</exception>
    public void AddOrder(Order order)
    {
        Client client = (from c in Clients where c.ClientID == order.ClientID select c).FirstOrDefault();
    ...
    /// <summary>
    /// Удаляет блюдо из меню и все связанные с ним позиции заказов.
    /// </summary>
    /// <param name="dishId">Идентификатор блюда.</param>
    /// <returns>true, если удаление выполнено успешно; иначе false.</returns>
    public bool RemoveMenuItem(int dishId)
    {
        Menu menuItem = (from m in Menu where m.DishID == dishId select m).FirstOrDefault();
        ...
        List<OrderItem> orderItemsToRemove = (from oi in OrderItems where oi.DishID == dishId select oi).ToList();
    ...
    /// </summary>
    /// <param name="menuItem">Блюдо для добавления.</param>
    ...
    {
        OrderItem orderItem = (from oi in OrderItems
    ...
    /// </summary>
    /// <param name="orderItem">Позиция заказа для добавления.</param>
    /// <exception cref="ArgumentException">Выбрасывается, если заказ или блюдо не существуют.</exception>
    public void AddOrderItem(OrderItem orderItem)
    {
        Order order = (from o in Orders where o.OrderID == orderItem.OrderID select o).FirstOrDefault();
        ...
        Menu menuItem = (from m in Menu where m.DishID == orderItem.DishID select m).FirstOrDefault();
    ...
    public void SaveToExcel()
    {
        try
        {
            ...
            SaveClientsSheet(workbook); // Принцип единой отвественности
            SaveOrdersSheet(workbook);
            SaveOrderItemsSheet(workbook);
            SaveMenuSheet(workbook);
    ...
    /// <summary>
    /// Сохраняет лист клиентов.
    /// </summary>
    /// <param name="workbook">Книга Excel.</param>
    private void SaveClientsSheet(Workbook workbook)
    {
    ...
    /// <summary>
    /// Сохраняет лист заказов.
    /// </summary>
    /// <param name="workbook">Книга Excel.</param>
    private void SaveOrdersSheet(Workbook workbook)
    {
    ...
    /// <summary>
    /// Сохраняет лист состава заказов.
    /// </summary>
    /// <param name="workbook">Книга Excel.</param>
    private void SaveOrderItemsSheet(Workbook workbook)
    {
    ...
    /// </summary>
    /// <param name="workbook">Книга Excel.</param>
    private void SaveMenuSheet(Workbook workbook)
    {
